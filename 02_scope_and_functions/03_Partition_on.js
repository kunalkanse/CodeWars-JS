...
function partitionOn(pred, items) {
  ...
  const trueFalseArr = items.reduce(
    (accArr, element) => {
      pred(element) ? accArr[0].push(element) : accArr[1].push(element);

      return accArr;
    },[[], []]);  // accArr[0] = true elements , acc_arr[1] => false elements

  items.length = 0;
  items = items.push(...trueFalseArr[1], ...trueFalseArr[0]);
  return trueFalseArr[1].length;
}
